...
def display_probs_mp(hand):
    combos = [
        list(c)
        for c in tqdm(
            combinations(remaining_cards, 7 - len(hand)),
    ...
    for offset, i in enumerate(range(0, len(combos), chunk_size)):
        ...
        p = mp.Process(
            target=calc_buckets_mp,
            args=(hand, combos[i : i + chunk_size], q, offset, lock),
        )
        p.start()
        processes.append(p)
...
def gen_self_combos(hand):
    ...
    return [
        list(c)
        for c in tqdm(
            combinations(remaining_cards, num_remaining),
...
def gen_opp_combos():
    return [
        list(c)
        for c in tqdm(
            combinations(remaining_cards, 2), desc="generating combos", leave=False
        )
    ]
...
def display_probs_mp_win_loss(hand):
    ...
    if len(hand) < 7:
        ...
        for offset, i in enumerate(range(0, len(combos), chunk_size)):
            p = mp.Process(
                target=calc_self_and_opp_buckets_and_win_counter_mp,
                args=(hand, combos[i : i + chunk_size], q, offset, lock),
            )
            p.start()
            processes.append(p)
        ...
        for i in range(0, len(combos), chunk_size):
            p = mp.Process(
                target=final_calc, args=(hand, combos[i : i + chunk_size], q)
            )
            p.start()
            processes.append(p)
    else:
    ...
    for i in range(len(self_probs) - 1, -1, -1):
        fancy_out(
            f"{num_to_hand[i]:.<16}: = {np.round(self_probs[i],7):<20}{num_to_hand[i]:.<16}: \u2248 {np.round(opp_probs[i],7)}"
        )
...
def calc_self_and_opp_buckets_and_win_counter_mp(
    hand, combos, q: mp.SimpleQueue, offset: int, lock
):
    """
...
def main(args, unknown):
    ...
    for next_card in [c for c in args.cards] + [c for c in unknown]:
        ...
        if next_card not in str_remaining:
            print(
                f"skipping: {next_card} is not a valid card. run with flag --help or -h to see valid card strings."
            )
        else:
    ...
    while not terminated:
        try:
            ...
            if len(hand) > 1:
                ...
                fancy_out(
                    str(np.array(best_hand, dtype=int))
                    + " -> "
                    + interpret_hand(best_hand, hand)
                )
            if len(hand) >= 7:
                break
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog="Poker Trick Helper", description=prog_desc)
