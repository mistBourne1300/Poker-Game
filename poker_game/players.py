...
class player:
    ...
    def decide(
        ...
        if bet_amount + player_bids[player_turn] > max_others_worth:
            ...
            try:
                print(
                    f"player {self.name} bet more than everyone else has. truncating bet to maximum of other's worth"
                )
            except:
                print(
                    f"player {self.name} bet more than everyone else has. truncating bet to maximum of other's worth"
        ...
        if bet_amount > self.money:
            print(
                f"player {self.name} tried to cheat by betting more than they have. They're all in now!"
            )
            bet_amount = self.money
            self.money = 0
        elif 0 < bet_amount < call_amount and bet_amount < self.money:
            print(
                f"player {self.name} tried to cheat by not going all in! As a consequence, they fold."
            )
            bet_amount = 0
        else:
...
class human(player):
    ...
    def __get_input(self, call_amount: int):
        ...
        def valid_choice():
            if (
                bet_amount >= call_amount
                or (bet_amount == self.money and bet_amount < call_amount)
                or bet_amount == 0
            ):
                return True
            return False
...
class random(player):
    ...
    def make_decision(
        ...
        if decision < call_perc:
            ...
            amount = (
                np.random.choice(
                    [i for i in range(call_amount, self.money + 1)], p=probability_dist
                )
...
class tracker(player):
    ...
    def make_decision(
        ...
        for i, name in enumerate(player_names):
            ...
            dump = {
                ...
                "tabled_cards": tuple(tabled_cards),
                "worth": int(others_worth[i]),
                "pot": int(pot),
                "bid": int(player_bids[i]),
            }
    ...
    def compute_results(
        self, auth, tabled_cards, others_worth, pot, player_names, player_cards
    ):
        if self.hash_auth(auth) != self.auth:
            return 0
...
class expector(player):
    ...
    def calculate_probs(self, auth, tabled_cards: list, num_players: int):
        if self.hash_auth(auth) != self.auth:
            return 0
        num_opps = self.num_opps_to_calculate(
            auth=auth, num_tabled=len(tabled_cards), num_players=num_players
        )
        ...
        speak_calculating = (len(tabled_cards) == 0) or (
            len(tabled_cards) < 5 and num_opps > 1
        )
        ...
        self.prev_probs = calc_probs_multiple_opps(
            hand=self.hand, tabled=tabled_cards, num_opps=num_opps
        )
        ...
        if num_opps != num_players - 1:
            if num_opps > 0:
                prob_win_against_true_opp_num = self.prev_probs[2][0] ** (
                    (num_players - 1) / num_opps
                )  # better estimate prob that we win
                within_hands_true_probs_win = self.prev_probs[3][:, 0] ** (
                    (num_players - 1) / num_opps
                )  # better estimate of within hands probs we win
            else:
                prob_win_against_true_opp_num = self.prev_probs[2][0] ** (
                    num_players - 1
                )  # better estimate prob that we win
            ...
            )  # get difference between old estimate and better estimae
            self.prev_probs[2][
                0
            ] = prob_win_against_true_opp_num  # make the prob we win the better estimate
            self.prev_probs[2][
                1
            ] += missing_prob  # give the prob we lose the difference
    ...
    def get_choice(self, auth, call_amount, tabled_cards, pot, player_bids, probs):
        ...
        expected_winnings = np.array(
            [
                probs[0] * (pot + num_betting_rounds_left * sum(player_bids))
                + probs[1] * (-i)
                + probs[2] * (pot + num_betting_rounds_left * sum(player_bids) + i) / 2
                for i in range(call_amount, self.money + 1)
            ]
        )
        ...
        if len(pos_expected_winnings) > 0:
            ...
            [
                print(f"bet: {i+call_amount}, E[{(i+call_amount)}]: {e}")
                for i, e in enumerate(pos_expected_winnings)
            ]
            bet_choice_probs = pos_expected_winnings**2
            print("pre softmax bet probs:")
            [
                print(f"bet: {i+call_amount}, p: {p}")
                for i, p in enumerate(bet_choice_probs)
            ]
            bet_choice_probs = pos_expected_winnings**2
            ...
            choice = call_amount + np.random.choice(
                [i for i in range(len(bet_choice_probs))], p=bet_choice_probs
            )
            ...
            [
                print(f"bet: {i+call_amount}, E[{(i+call_amount)}]: {e}")
                for i, e in enumerate(expected_winnings)
            ]
            choice = 0
        else:
            ...
            expected_winnings = (
                ...
                + probs[1] * (-choice)
                + probs[2]
                * (pot + num_betting_rounds_left * sum(player_bids) + choice)
                / 2
            )
    ...
    def make_decision(
        ...
        _, _, probs, _ = self.calculate_probs(
            auth, tabled_cards=tabled_cards, num_players=num_players
        )
    ...
    def compute_results(
        ...
        return super().compute_results(
            auth, tabled_cards, others_worth, pot, player_names, player_cards
        )
...
class ratio(expector):
    ...
    def get_choice(self, auth, call_amount, tabled_cards, pot, player_bids, probs):
        ...
        expected_winnings = np.array(
            [
                probs[0] * (expected_pot)
                + probs[1] * (-i)
                + probs[2] * (expected_pot + i) / 2
                for i in range(call_amount, self.money)
            ]
        )
        expected_loss_ratios = np.array(
            [
                probs[0] * (-(expected_pot) / (self.money + expected_pot))
                + probs[1] * (i / (self.money - i))
                + probs[2] * (-expected_pot / (2 * self.money + expected_pot))
                for i in range(call_amount, self.money)
            ]
        )
        pos_expected_winnings = expected_winnings[
            (expected_loss_ratios < 0) & (expected_winnings > 0)
        ]
        ...
        if len(pos_expected_winnings) > 0:
            ...
            [
                print(f"bet: {i+call_amount}, E[{(i+call_amount)}]: {e}")
                for i, e in enumerate(expected_loss_ratios)
            ]
            ...
            max_only_expected_winnings = only_expected_winnings[
                only_expected_winnings_argmax
            ]
            [
                print(f"bet: {i+call_amount}, E[{(i+call_amount)}]: {e}")
                for i, e in enumerate(only_expected_winnings)
            ]
            ...
            if len(expected_winnings) > 0:
                ...
                max_expected_winnings = (
                    probs[0] * expected_pot
                    + probs[1] * (-self.money)
                    + probs[2] * (expected_pot + self.money) / 2
                )
...
class bayesian(ratio):
    def __init__(
        self, name, auth, bayes_avg=False, ignore_ties=False, lamb_multi=False
    ):
        super().__init__(name, auth)
    ...
    def get_bayesian_wl_probs(
        ...
        num_players = len(player_names) - sum(folded_players)
        self_hand_probs, opp_hand_probs, wl_probs, within_hands_wl_probs = (
            self.calculate_probs(
                auth=auth, tabled_cards=tabled_cards, num_players=num_players
            )
        )
        ...
        if not os.path.exists(os.path.join(self.folder, self.temp_filename)):
            ...
            for i, name in enumerate(player_names):
                if i == player_turn:
                    continue
                temp_data[name] = {
                    "pre-flop": {"called": tuple(), "raised": tuple()},
                    "post-flop pre-turn": {"called": tuple(), "raised": tuple()},
                    "post-turn pre-river": {"called": tuple(), "raised": tuple()},
                    "post-river": {"called": tuple(), "raised": tuple()},
                }
        else:
            ...
            if len(tabled_cards) == 0:
                ...
                try:
                    with open(
                        os.path.join(self.folder, self.temp_filename), "r+"
                    ) as temp_file:
        ...
        for name in player_names:
            ...
            if not os.path.exists(player_path):
                ...
                obj = {
                    "pre-flop": {"called": dict(), "raised": dict()},
                    "post-flop pre-turn": {"called": dict(), "raised": dict()},
                    "post-turn pre-river": {"called": dict(), "raised": dict()},
                    "post-river": {"called": dict(), "raised": dict()},
                }
        ...
        for i in range(len(player_names)):
            ...
            player_bid_type = (
                "raised" if i == last_raise_idx and player_bid > 0 else "called"
            )
            bid_str = ""
            ...
            for i, p in enumerate(tie_probs):
                if (
                    np.any(within_hands_wl_probs[i, :] == np.nan)
                    or np.any(within_hands_wl_probs[i, :] == np.inf)
                    or np.any(within_hands_wl_probs[i, :] == -np.inf)
                ):
                    continue
            ...
            if new_wl_probs[0] < wl_probs[0] and not (
                np.any(new_wl_probs == np.nan)
                or np.any(new_wl_probs == np.inf)
                or np.any(new_wl_probs == -np.inf)
            ):
    ...
    def make_decision(
        ...
        probs = self.get_bayesian_wl_probs(
            auth=auth,
            call_amount=call_amount,
            tabled_cards=tabled_cards,
            others_worth=others_worth,
    ...
    def compute_results(
        ...
        for i in range(len(player_names)):
            ...
            with open(
                os.path.join(player_path, self.database_filename), "r+"
            ) as datafile:
                player_data = json.load(datafile)
            ...
            for betting_round in temp_player_dict.keys():
                ...
                player_hand_opps, _, _, _ = calc_probs_multiple_opps(
                    hand=player_hand, tabled=tables, num_opps=0
                )
                for bet_type in betting_round_dict.keys():
                    ...
                    for bet_amount in bet_type_tuple:
                        if bet_amount in player_data[betting_round][bet_type]:
                            bet_amount_tuples = player_data[betting_round][bet_type][
                                bet_amount
                            ]
                            player_data[betting_round][bet_type][bet_amount] = (
                                *bet_amount_tuples,
                                tuple(player_hand_opps),
                            )
                        else:
                            player_data[betting_round][bet_type][bet_amount] = tuple(
                                (
                                    tuple(np.zeros_like(player_hand_opps)),
                                    tuple(player_hand_opps),
                                )
                            )

            with open(
                os.path.join(player_path, self.database_filename), "w"
            ) as datafile:
