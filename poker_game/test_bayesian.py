...
import numpy
from players import bayesian, expector, player
...
def generate_table_w_players(player_class: player):
    ...
    t = table(
        [bayesian.constructor] * num_players,
        [str(i) for i in range(num_players)],
        starting_money=10,
        say=print,
    )
    auths = t.auths

    for i, (bayes_avg, ignore_ties, lamb_multi) in enumerate(
        itertools.product([True, False], repeat=3)
    ):
        new_player = bayesian.constructor(
            f"bayes{i}",
            auths[i],
            bayes_avg=bayes_avg,
            ignore_ties=ignore_ties,
            lamb_multi=lamb_multi,
        )
