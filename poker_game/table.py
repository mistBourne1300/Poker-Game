...
class table:
    def __init__(
        self,
        player_constructors: list,
        player_names: list,
        starting_money: int,
        small_blind: int = 1,
        big_blind: int = None,
        say: callable = utils.say,
        verbose=True,
    ):
    ...
    def initiate_blinds(self, dealer_idx):
        ...
        current_bids[small_blind_index] = small_blind_player.blind(
            small_blind_auth, self.small_blind
        )
        if (
            current_bids[small_blind_index] < self.small_blind
            or small_blind_player.worth() == 0
        ):
            player_max_earnings[small_blind_index] = current_bids[
                small_blind_index
            ] * len(self.players)
            self.say(
                f"{small_blind_player.get_name()} is all in with {current_bids[small_blind_index]} for small blind.\nTheir max earning potential is now {player_max_earnings[small_blind_index]}"
            )
        (
            utils.confirm(
                f"{small_blind_player.get_name()} pays small blind of {current_bids[small_blind_index]}"
            )
        ...
        current_bids[big_blind_index] = big_blind_player.blind(
            big_blind_auth, self.big_blind
        )
        if (
            current_bids[big_blind_index] < self.big_blind
            or big_blind_player.worth() == 0
        ):
            player_max_earnings[big_blind_index] = current_bids[big_blind_index] * len(
                self.players
            )
            self.say(
                f"{big_blind_player.get_name()} is all in with {current_bids[big_blind_index]} for big blind.\nTheir max earning potential is now {player_max_earnings[big_blind_index]}"
            )
        if current_bids[big_blind_index] < self.big_blind:
            if len(self.players) == 2:
                self.say(
                    f"since there are only 2 players, and one is all in, bet amount is now {max(current_bids)}"
                )
        ...
        (
            utils.confirm(
                f"{big_blind_player.get_name()} pays big blind of {current_bids[big_blind_index]}"
            )
            if self.MODE == mode.HYBRID_PLAY
            else self.say(
                f"{big_blind_player.get_name()} pays big blind of {current_bids[big_blind_index]}"
    ...
    def num_betting_players(self, folded_players: list, player_max_earnings: list):
        return (
            ...
            - sum(
                [
                    (earning_potential < np.inf)
                    for earning_potential in player_max_earnings
                ]
            )
        )
    ...
    def continue_betting(self, current_bids, folded_players, player_max_earnings):
        ...
        for i in range(len(folded_players)):
            if folded_players[i]:
                continue
            if player_max_earnings[i] < np.inf:
                continue
    ...
    def pre_flop_bet(self, dealer_idx):
        current_bids, player_max_earnings, current_raise = self.initiate_blinds(
            dealer_idx
        )
        # current_raise = self.big_blind
        ...
        while current_player_idx != big_blind_idx:
            ...
            #     break
            if (
                folded_players[current_player_idx]
                or player_max_earnings[current_player_idx] < np.inf
            ):
            ...
            player_bid = current_player.decide(
                auth=current_auth,
                call_amount=call_amount,
                tabled_cards=[],
                others_worth=others_worth,
                pot=0,
                player_bids=current_bids,
                player_turn=current_player_idx,
                player_names=[player.get_name() for player in self.players],
                folded_players=folded_players,
                last_raise_idx=last_raise_idx,
                prev_raise_idx=self.prev_raise_idx,
                bet_num=roundabout,
            )
            ...
            if player_bid < call_amount:
                ...
                self.say(
                    f"{current_player.get_name()} raises. bid is now {current_raise}"
                )
            else:
                current_bids[current_player_idx] += player_bid
                if player_bid > 0:
                    ...
                    confirm_needed = False
            if current_player.worth() == 0:
                player_max_earnings[current_player_idx] = pot + (
                    current_bids[current_player_idx]
                ) * self.num_betting_players(
                    folded_players=folded_players,
                    player_max_earnings=player_max_earnings,
                )
                self.say(
                    f"{current_player.get_name()} goes all in.\nTheir maximum earning potential is {player_max_earnings[current_player_idx]}"
                )
            if (
                (not isinstance(current_player, human))
                and current_raise > 0
                and self.MODE == mode.HYBRID_PLAY
                and confirm_needed
            ):
                utils.confirm("")
            current_player_idx = (current_player_idx + 1) % len(self.players)
        ...
        if (
            self.num_betting_players(
                folded_players=folded_players, player_max_earnings=player_max_earnings
            )
            <= 1
        ):
        ...
        confirm_needed = True
        if self.v:
            print(f"{self.players[big_blind_idx].get_name()}'s turn")
        ...
        others_worth = [player.worth() for player in self.players]
        player_bid = big_blind_player.decide(
            auth=big_blind_auth,
            call_amount=call_amount,
            tabled_cards=[],
            others_worth=others_worth,
        ...
        if player_bid < call_amount:
            ...
            self.say(
                f"{big_blind_player.get_name()} raises. bid is now {current_raise}"
            )
        else:
            current_bids[big_blind_idx] += player_bid
            if player_bid > 0:
                ...
                confirm_needed = False
        if big_blind_player.worth() == 0:
            player_max_earnings[big_blind_idx] = pot + (
                current_bids[big_blind_idx]
            ) * self.num_betting_players(
                folded_players=folded_players, player_max_earnings=player_max_earnings
            )
            self.say(
                f"{big_blind_player.get_name()} goes all in.\nTheir maximum earning potential is {player_max_earnings[current_player_idx]}"
            )
        if (
            (not isinstance(big_blind_player, human))
            and current_raise > 0
            and self.MODE == mode.HYBRID_PLAY
            and confirm_needed
        ):
        ...
        if current_raise > self.big_blind:
            while self.continue_betting(
                ...
                folded_players=folded_players,
                player_max_earnings=player_max_earnings,
            ):  # current_player_idx != last_raise_idx:
                confirm_needed = True
                # if self.num_betting_players(folded_players=folded_players, player_max_earnings=player_max_earnings) <= 1 and current_bids[current_player_idx] == current_raise:
                #     break
                if (
                    ...
                    if self.v:
                        print(
                            f"{self.players[current_player_idx].get_name()} cannot bet"
                        )
                    current_player_idx = (current_player_idx + 1) % len(self.players)
                    continue
                if self.v:
                ...
                if player_bid < call_amount:
                    if current_player.worth() > 0:
                        folded_players[current_player_idx] = True
                        current_player_idx = (current_player_idx + 1) % len(
                            self.players
                        )
                        self.say(f"{current_player.get_name()} folds.")
                        confirm_needed = False
    ...
    def post_flop_bet(
        ...
        while current_player_idx != stop_idx or do:
            ...
            if (
                ...
                if self.v:
                    print(
                        f"{self.players[current_player_idx].get_name()} cannot bet. skipping"
                    )
                current_player_idx = (current_player_idx + 1) % len(self.players)
    ...
    def distribute_wealth(self, pot, folded_players, player_max_earnings, tabled_cards):
        if sum(folded_players) == len(self.players) - 1:
            ...
            (
                ...
                if self.MODE == mode.HYBRID_PLAY
                else self.say(f"{winner.get_name()} wins {pot} moneys")
            )
        ...
        for idx in range(len(self.players)):
            ...
            (
                ...
                if self.MODE == mode.HYBRID_PLAY
                else self.say(f"{player.get_name()} reveals hand")
            )
            hand = player.reveal_hand(auth)
            player_cards[idx] = hand
            cardnames = utils.card_list_to_card_names(
                [utils.tuple_to_str(c) for c in hand]
            )
            self.say(
                f"{player.get_name()} reveals the {cardnames[0]} and the {cardnames[1]}"
            )
            hand = hand + tabled_cards
            player_hands[idx] = hand
            player_best_hands[idx] = utils.calc_best_hand(hand)
        winning_order = sorted(
            range(len(player_best_hands)), key=player_best_hands.__getitem__
        )[::-1]
        if self.v:
        ...
        for i in range(len(player_best_hands)):
            ...
            if folded_players[i]:
                if self.v:
                    print(f"{player.get_name()} folded")
            ...
            player_messages.append(
                f"{player.get_name()} has {handname} with {cardnames}"
            )
            if self.v:
                print(player_messages[-1])
        if self.v:
            print()
        ...
        while pot > 0:
            ...
            while tie_index < len(player_best_hands):
                if (
                    player_best_hands[winning_order[starting_winning_indexer]]
                    == player_best_hands[winning_order[tie_index]]
                ):
                    tie_index += 1
                else:
                    break
            ...
            if len(winning_player_indexes) == 1:
                if player_max_earnings[winning_player_indexes[0]] < pot:
                    ...
                    (
                        confirm(
                            f"{self.players[winning_player_indexes[0]].get_name()} wins {max_earning} moneys"
                        )
                        if self.MODE == mode.HYBRID_PLAY
                        else self.say(
                            f"{self.players[winning_player_indexes[0]].get_name()} wins {max_earning} moneys"
                    ...
                    (
                        confirm(
                            f"{self.players[winning_player_indexes[0]].get_name()} wins the pot of {pot} moneys"
                        )
                        if self.MODE == mode.HYBRID_PLAY
                        else self.say(
                            f"{self.players[winning_player_indexes[0]].get_name()} wins the pot of {pot} moneys"
                ...
                self.say(
                    f"we have a tie between {[self.players[winning_player_indexes[i]].get_name() for i in range(len(winning_player_indexes))]}"
                )
                ...
                while remainder > 0:
                    self.say(
                        f"{pot} cannot be divided equally among {len(winning_player_indexes)} players."
                    )
                    remainder_doling_starting_index = np.random.randint(
                        len(winning_player_indexes)
                    )
                    self.say(
                        f"remainder of {remainder} will be doled out starting with {self.players[winning_player_indexes[remainder_doling_starting_index]].get_name()}."
                    )
                    while remainder > 0:
                        player_winnings[remainder_doling_starting_index] += 1
                        remainder -= 1
                        remainder_doling_starting_index = (
                            remainder_doling_starting_index + 1
                        ) % len(player_winnings)
                    ...
                    for i, winner_idx in enumerate(winning_player_indexes):
                        ...
                        if max_earnings < np.inf:
                            self.say(
                                f"{player.get_name()} can only win {max_earnings}. the rest will be redistributed"
                            )
                ...
                for i, idx in enumerate(winning_player_indexes):
                    ...
                    (
                        ...
                        if self.MODE == mode.HYBRID_PLAY
                        else self.say(f"{player.get_name()} wins {winnings} moneys")
                    )
    ...
    def play_game(self):
        ...
        while len(self.players) > 1:
            ...
            remaining_deck, str_remaining, strranks, strsuits, num_to_hand = (
                utils.create_deck()
            )
            tabled_cards = []
            ...
            for i, player in enumerate(self.players):
                if not isinstance(player, human):
                    ...
                    if self.MODE == mode.HYBRID_PLAY:
                        ...
                        utils.computer_add(
                            cards,
                            str_remaining=str_remaining,
                            remaining_cards=remaining_deck,
                            max_size=2,
                        )
            ...
            pot, folded_players, player_max_earnings, last_raise_idx = (
                self.pre_flop_bet(dealer_idx)
            )
            ...
            if sum(folded_players) >= len(self.players) - 1:
                ...
                self.distribute_wealth(
                    pot, folded_players, player_max_earnings, tabled_cards
                )
            ...
            while len(tabled_cards) < 3:
                if self.MODE == mode.HYBRID_PLAY:
                    if self.v:
                        print(
                            f"current tabled cards: {[utils.tuple_to_str(c) for c in tabled_cards]}"
                        )
                    say(
                        f"{self.players[dealer_idx].get_name()}, deal the flop. Enter cards: "
                    )
            ...
            while len(tabled_cards) < 4:
                if self.MODE == mode.HYBRID_PLAY:
                    ...
                    say(
                        f"{self.players[dealer_idx].get_name()}, deal the turn. Enter card: "
                    )
            ...
            while len(tabled_cards) < 5:
                if self.MODE == mode.HYBRID_PLAY:
                    ...
                    say(
                        f"{self.players[dealer_idx].get_name()}, deal the river. Enter card: "
                    )
            ...
            if (
                ...
                pot, folded_players, player_max_earnings, _ = self.post_flop_bet(
                    pot,
                    folded_players,
                    player_max_earnings,
                    tabled_cards,
        ...
        while game_on:
            ...
            try:
                ...
                while not found_valid_input:
                    ...
                    print(
                        f"player order: {[player.get_name() for player in self.players]}"
                    )
                    ...
                    [
                        print(player.get_name(), player.worth())
                        for player in self.players
                    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--names",
        nargs="*",
        help="player names. if not enough are specified, will prompt for more",
    )
    parser.add_argument(
        "--classes",
        nargs="*",
        help="the corresponding player classes. if not enough are specified, or if a player subclass cannot be found with the specified name, will prompt for more",
    )
    ...
    t = table(
        player_constructors=player_constructors,
    ...
    try:
        ...
        os.system("clear")
        fancy_out("process terminated", sleep_time=0.03)
        time.sleep(0.3)
        os.system("clear")
